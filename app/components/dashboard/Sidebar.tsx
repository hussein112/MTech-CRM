...
const NAV_ITEMS = [
  ...
  { label: "Resources", href: "/resources", icon: "folder_shared" },
  { label: "File Storage", href: "/storage", icon: "backup" },
  { label: "Password Manager", href: "/passwords", icon: "passkey" },
  { label: "Changelog", href: "/changelog", icon: "campaign" },
...
export function Sidebar() {
  ...
  return (
    <>
            ...
            if (collapsed) {
              return (
                <div key={item.label} style={{ marginBottom: 2 }}>
                  ...
                  >
                    <span className="material-symbols-outlined" style={{ fontSize: 20, color: isActive ? "var(--accent-crm)" : "var(--text2)" }}>
                      {item.icon}
                    </span>
                  </Link>
                </div>
                  ...
                  {hasChildren ? (
                    <>
                      ...
                      <span
                        ...
                        style={{ fontSize: 16, color: "var(--text2)", transition: "transform .2s", transform: isExpanded ? "rotate(180deg)" : "none", flexShrink: 0, pointerEvents: "none" }}
                      >expand_more</span>
                    </>
                  ) : (
                    ...
                    {item.children!.map(child => {
                      ...
                      return (
                        <Link
                          key={child.href}
                          href={child.href}
                          style={{ display: "block", padding: "5px 10px", fontSize: 12, fontWeight: childActive ? 700 : 500, color: childActive ? "var(--accent-crm)" : "var(--text2)", textDecoration: "none", borderRadius: 6, marginBottom: 2, transition: ".15s" }}
                          onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = "var(--text)" }}
                          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = childActive ? "var(--accent-crm)" : "var(--text2)" }}
                        >
                          {child.label}
                        </Link>
                      )
      ...
      >
        <span className="material-symbols-outlined" style={{ fontSize: 14, color: "var(--text2)" }}>
